...
let RazorpayService = class RazorpayService {
    ...
    async createOrder(params) {
        ...
        if (!Number.isFinite(amountInPaise) || amountInPaise < 100) {
            throw new common_1.BadRequestException('Razorpay payment amount must be at least ₹1.');
        }
        if (this.keyId.includes('mockkey') || this.keySecret.includes('mocksecret')) {
            throw new common_1.BadRequestException('Razorpay keys are not configured. Add valid Razorpay keys to the backend environment.');
        }
        ...
        try {
            ...
            throw new common_1.BadRequestException('Unable to reach Razorpay. Please check the backend internet connection and try again.');
        }
        if (!response.ok) {
            const errorData = await response.json().catch(() => null);
            throw new common_1.BadRequestException(errorData?.error?.description || 'Razorpay could not create the payment order.');
        }
